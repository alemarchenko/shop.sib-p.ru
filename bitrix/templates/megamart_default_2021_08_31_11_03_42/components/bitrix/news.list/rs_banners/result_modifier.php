...
$arResult['BANNER_HEIGHT'] = $arResult['BANNER_TYPE'] == 'underlay' ? '660px' : '400px';
$arResult['IS_JS_HEIGHT_ADJUST'] = 'N';
$arResult['MARGIN_TOP'] =  null;

$arResult['BANNER_CLASS'] = $arResult['BANNER_TYPE'] == 'wide' || $arResult['BANNER_TYPE'] == 'underlay' ? '__wide' : '__center container';
if ($arResult['BANNER_TYPE'] == 'underlay')
	$arResult['BANNER_CLASS'].= ' __underlay';

$arResult['BANNER_OPTIONS'] = array(
	"autoplay" =>  ($arParams['RS_BANNER_IS_AUTOPLAY'] == 'N') ? false : true,
	"autoplay-speed" => (!empty($arParams['RS_BANNER_AUTOPLAY_SPEED'])) ? $arParams['RS_BANNER_AUTOPLAY_SPEED'] : 1000,
...
if(!empty($arParams['RS_SIDEBANNERS_IBLOCK_ID']) && !empty($arParams['RS_SIDEBANNERS'])) {
	...
	$dbSidebanners = CIBlockElement::GetList(
		...
		false,
		array(
			"ID", "IBLOCK_ID", "NAME", "PROPERTY_IMAGE", "PROPERTY_LINK", "PROPERTY_PLACE"
		)
	);
	...
	while($arSidebanner = $dbSidebanners->Fetch()) {
		...
		$arResult['SIDEBANNERS'][$place][] = array(
			...
			"link" => $arSidebanner['PROPERTY_LINK_VALUE']
		);
	}
}
