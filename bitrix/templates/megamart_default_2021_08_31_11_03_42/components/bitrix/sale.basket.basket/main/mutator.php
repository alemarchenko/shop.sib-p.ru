...
use Redsign\MegaMart\Sale\DiscountService;
use Redsign\MegaMart\Sale\DeliveryDiscountPrediction;
...
$result['CUSTOM_ERROR_MESSAGES'] = [];
$result['CUSTOM_INFO_MESSAGES'] = [];
		...
		foreach ($row['SKU_DATA'] as $skuBlock)
		{
			...
			foreach ($skuBlock['VALUES'] as $skuItem)
			{
				...
				)
				{
					...
					$skuBlockData['SELECT_VALUE'] = $skuItem['NAME'];
				}
...
if (\Bitrix\Main\Loader::includeModule('redsign.megamart'))
{
	...
	{
		$allowOrder = \Redsign\Megamart\OrderUtils::checkMinPrice($result['allSum']);
	}
	...
	if (!$allowOrder)
	{
		$sErrorMessage = \Bitrix\Main\Config\Option::get(
			...
			Loc::getMessage('SBB_MIN_PRICE_ERROR')
		);
		...
		$arReplacements = [
			$nMinPrice,
			$result['allSum'],
			abs($result['allSum'] - $nMinPrice)
		];

		$sErrorMessage = str_replace($arSearch, $arReplacements, $sErrorMessage);

		$result['CUSTOM_ERROR_MESSAGES'][] = $sErrorMessage;
	}
}
...
$discountContext = new \Bitrix\Sale\Discount\Context\User($USER);
$discountService = new DiscountService($discountContext, SITE_ID);
$deliveryDiscountPrediction = new DeliveryDiscountPrediction($discountService);
$prediction = $deliveryDiscountPrediction->getFirstMatching($basket);
if (!empty($prediction))
{
	if ($prediction['DISCOUNT_UNIT'] === DeliveryDiscountPrediction::PERC_UNIT)
	{
		if ($prediction['DISCOUNT'] == 100)
		{
			$result['CUSTOM_INFO_MESSAGES'][] = Loc::getMessage('SBB_FREE_DELIVERY_PREDICTION', [
				'#PRICE#' => CCurrencyLang::CurrencyFormat($prediction['VALUE'] - $result['allSum'], $result['CURRENCY'])
			]);
		}
		else
		{
			$result['CUSTOM_INFO_MESSAGES'][] = Loc::getMessage('SBB_DISCOUNT_DELIVERY_PREDICTION', [
				'#PRICE#' => CCurrencyLang::CurrencyFormat($prediction['VALUE'] - $result['allSum'], $result['CURRENCY']),
				'#FROM_PRICE#' => CCurrencyLang::CurrencyFormat($prediction['VALUE'], $result['CURRENCY']),
				'#PERCENT#' => $prediction['DISCOUNT']
			]);
		}
	}
}
...
$totalData = array(
	'DISABLE_CHECKOUT' => !$allowOrder || (int)$result['ORDERABLE_BASKET_ITEMS_COUNT'] === 0,
	'PRICE' => $result['allSum'],
