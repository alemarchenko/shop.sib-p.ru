	...
	<ul class="nav nav-tabs"><?
		foreach ($arData as $tabId => $arTab)
		{
			if (isset($arTab["NAME"]) && isset($arTab["CONTENT"]))
			{
				...
				?><li id="<?=$id?>"<?=($tabActive ? ' class="active"' : '')?>><a rel="nofollow" href="#<?=$id?>_cont" data-toggle="tab"><?=$arTab["NAME"]?></a></li><?
				if($tabActive || $activeTabId == "")
					$activeTabId = $tabId;

				$content .= '<div id="'.$id.'_cont" class="tab-pane'.($tabActive ? ' show active' : '').'">'.$arTab["CONTENT"].'</div>';
				$tabIDList[] = $tabId;
			}
		}
		unset($tabId, $arTab);
...
<?php
		...
		foreach ($arData as $tabId => $arTab)
		{
			if (isset($arTab["NAME"]) && isset($arTab["CONTENT"]))
			{
				...
				?>
				<div id="<?=$id?>_cont" class="tab-pane<?=($tabActive ? ' show active' : '')?>">
					<?php
