...
// $arParams['PRODUCT_PREVIEW'] = 'N';

Loc::loadMessages(__FILE__);

Loader::includeModule('redsign.megamart');
...
if (isset($arResult['ITEM']))
{
	...
	$itemIds = array(
		...
		'PRICE_ECONOMY' => $areaId.'_price_economy',
...
{
	$measureRatio = 1;
	$actualItem['ITEM_MEASURE']['TITLE'] = Loc::getMessage('RS_MM_BCI_CATALOG_ITEM_MEASURE');
	$item['ITEM_MEASURE_RATIO_SELECTED'] = 0;
	$item['ITEM_MEASURE_RATIOS'][$item['ITEM_MEASURE_RATIO_SELECTED']]['RATIO'] = 1;

	$price = $item['RS_PRICES'];
	$item['CAN_BUY'] = $actualItem['CAN_BUY'] = is_array($price);
	$item['PRODUCT']['TYPE'] = 0;
}
...
/*
	$discountPositionClass = isset($arResult['BIG_DISCOUNT_PERCENT']) && $arResult['BIG_DISCOUNT_PERCENT'] === 'Y'
		? 'product-cat-label-big'
		: 'product-cat-label-small';
*/
...
*/
	...
	$buttonSizeClass = isset($arResult['BIG_BUTTONS']) && $arResult['BIG_BUTTONS'] === 'Y' ? 'btn-md' : 'btn-sm btn-rounded';
	$itemHasDetailUrl = isset($item['DETAIL_PAGE_URL']) && $item['DETAIL_PAGE_URL'] != '';
	if ($arParams['HIDE_DETAIL_LINKS'] == 'Y')
		$itemHasDetailUrl = false;

	$containerClass = 'product-cat-container';
	?>

	<div class="<?=$containerClass?>" id="<?=$areaId?>" data-entity="item">
		<?
		...
		if (!$haveOffers)
		{
			$jsParams = array(
				...
				'PRODUCT_PREVIEW' => $arParams['PRODUCT_PREVIEW'] === 'Y',
				...
				'VISUAL' => array(
					...
					'PRICE_ECONOMY_ID' => $itemIds['PRICE_ECONOMY'],
					...
					'FAVORITE_ID' => $itemIds['FAVORITE_LINK'],
				),
				'MESS' => array(
					'MESS_BTN_BUY' => ($arParams['ADD_TO_BASKET_ACTION'] === 'BUY' ? $arParams['MESS_BTN_BUY'] : $arParams['MESS_BTN_ADD_TO_BASKET']),
		...
		else
		{
			$jsParams = array(
				...
				'VISUAL' => array(
					...
					'COMPARE_LINK_ID' => $itemIds['COMPARE_LINK'],
					'SUBSCRIBE_ID' => $itemIds['SUBSCRIBE_LINK'],
					'PRODUCT_DEAL_ID' => $itemIds['PRODUCT_DEAL'],
					'FAVORITE_ID' => $itemIds['FAVORITE_LINK'],
				),
				'BASKET' => array(
					'QUANTITY' => $arParams['PRODUCT_QUANTITY_VARIABLE'],
		...
		$jsParams['SLIDER_SLIDE_COUNT'] = $arParams['SLIDER_SLIDE_COUNT'];
