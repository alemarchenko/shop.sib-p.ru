...
;(function(window) {
	...
	RS.TuningComponent.prototype.documentReady = function() {
		...
		// default settings
		elements = tuningObj.querySelectorAll(this_.selectors.defaultSettings);
		if (elements && elements.length > 0)
		{
			for (i = 0; i < elements.length; i++)
			{
				elements[i].addEventListener('click', this_.restoreDefaultSettings);
			}
		}
		...
		// scroll
		elements = tuningObj.querySelectorAll('.rstuning__scroll');
		if (elements && elements.length > 0)
		{
			for (i = 0; i < elements.length; i++)
			{
	...
	RS.TuningComponent.prototype.windowKeyUp = function(e) {
		var this_ = rsTuningComponent;
			tuningObj = this_.getTuningComponent();
