...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/modules/iblock/admin/iblock_section_admin.php");
...
if($IB_ID>0){
    ...
    CAdminMessage::ShowMessage(
        Array(
        "TYPE"=>"ERROR",
        "MESSAGE" => GetMessage("EASYDIRECT_TEMPLATES_sections_ibid_err"),
        "HTML"=>true
        )
    );
...
if ($arID = $lAdmin->GroupAction()) ///GROUP ACT
{
	...
	if($_REQUEST['action']=='send_to_ed') {
	    ...
	    $_SESSION["WTCED_IS_SECTION_TO_CREATE_ADS"]="Y";
	    echo '
...
$arGroupActions['send_to_ed'] = GetMessage('EASYDIRECT_TEMPLATES_sections_sendid');
