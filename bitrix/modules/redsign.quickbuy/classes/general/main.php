...
class CRSQUICKBUYMain
{
	public static function OnBeforePrologElementUpdate() {
		...
		if(CRSQUICKBUYTab::NeedAddTab()) {
			$VALUE_TYPE = 'Perc';
			$VALUE_TYPE = ($_REQUEST['redsign_quickbuy_discount_type']=='CurEach'?'CurEach':$VALUE_TYPE);
			...
			while($arItems = $dbBasketItems->Fetch()) {
				...
				if($data = $res2->Fetch()) {
					...
					if($arFieldsNew['QUANTITY']<1) {
						$quickBuyElem->Delete($data['ID']);
					} else {
						$quickBuyElem->Update($data['ID'], $arFieldsNew, true, false);
					}
				} else {
