...
class Region
{
	...
	static function OnPageStart()
	{
		...
		if ($arRegionSaved)
		{
			...
			$arRegionCurrent = self::getById($arRegionSaved['ID']);
		}
		...
		if (is_array($arFilterIds) && count($arFilterIds) > 0)
		{
			...
			if (is_array($item['OFFERS']) && count($item['OFFERS']) > 0)
			{
				...
				foreach ($item['OFFERS'] as $iOfferKey => $arOffer)
				{
					...
					if (!$item['OFFERS'][$iOfferKey]['CAN_BUY'] || $item['OFFERS'][$iOfferKey]['ITEM_PRICE_SELECTED'] === null)
						continue;

					$currentPrice = $item['OFFERS'][$iOfferKey]['ITEM_PRICES'][$item['OFFERS'][$iOfferKey]['ITEM_PRICE_SELECTED']];
