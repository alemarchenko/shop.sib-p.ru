...
class RSDevFunc
{
	public static function Init($arInit)
	{
		...
		if (in_array('jsfunc',$arInit))
		{
			...
			$asset->addString('<script>var RSDevFunc_BasketEndWord_end1 = \''.Loc::getMessage("RSDF_END_1").'\';var RSDevFunc_BasketEndWord_end2 = \''.Loc::getMessage("RSDF_END_2").'\';var RSDevFunc_BasketEndWord_end3 = \''.Loc::getMessage("RSDF_END_3").'\';</script>');
			$asset->addJs($pathJS.'/tools.js');
		}
	}
	...
	public static function GetDataForProductItem(&$arItems, $params=array()) {
		...
		if(Loader::includeModule('iblock') && is_array($arItems) && count($arItems)>0) {
			...
			foreach ($arElements as $iElementId => $arElement)
			{
				...
				if ($CODE && isset($arElement['PROPERTIES'][$CODE]['VALUE']) && !is_array($arElement['PROPERTIES'][$CODE]['VALUE']) && IntVal($arElement['PROPERTIES'][$CODE]['VALUE']) > 0)
				{
					$arFile = $arFilesIds[$arElement['PROPERTIES'][$CODE]['VALUE']];
					$arFile['RESIZE'] = CFile::ResizeImageGet(
						$arElements[$iElementId]['PROPERTIES'][$CODE]['VALUE'],
						[
	...
	public static function getPriceMatrixEx(&$arElement, $filterQuantity = 0, $arFilterType = array(), $VAT_INCLUDE = 'Y', $arCurrencyParams = array()) {
		...
		if (empty($arElement['PRICE_MATRIX'])) {
			if ($bCatalog) {
	...
	public static function getElementPictures(&$arItem, $params, $iLimit = false)
	{
		...
		foreach ($arElements as $iElementKey => $arElement)
		{
			...
			)
			{
				...
				if (
					isset($arElement['DISPLAY_PROPERTIES'][$params['ADDITIONAL_PICT_PROP'][$arElement['IBLOCK_ID']]]['FILE_VALUE'])
				...
				else
				{
					...
					{
						if (
							!is_array($arElement['PROPERTIES'][$params['ADDITIONAL_PICT_PROP'][$arElement['IBLOCK_ID']]]['VALUE'])
	...
	public static function GetNoPhoto($arSizes = array()) {
		$return = false;
	...
	public static function BasketEndWord($num,$end1="",$end2="",$end3="") {
		if($end1=='') $end1 = Loc::getMessage('RSDF.END_1');
		if($end2=='') $end2 = Loc::getMessage('RSDF.END_2');
		if($end3=='') $end3 = Loc::getMessage('RSDF.END_3');
	...
	public static function getHighloadBlockValues(&$arProp) {
		/*
		if (RSDevFuncOffersExtension::$highLoadInclude === null)
			RSDevFuncOffersExtension::$highLoadInclude = Loader::includeModule('highloadblock');
		if (!RSDevFuncOffersExtension::$highLoadInclude)
			continue;
		*/
		Loader::includeModule('highloadblock');

		if (
			!isset($arProp['USER_TYPE_SETTINGS']['TABLE_NAME']) ||
			empty($arProp['USER_TYPE_SETTINGS']['TABLE_NAME'])
		) {
			return;
		}

		$highBlock = \Bitrix\Highloadblock\HighloadBlockTable::getList(
			array(
				'filter' => array(
		...
		$arProp['USER_TYPE_SETTINGS']['ENTITY'] = $entity;
		...
		$arProp['USER_TYPE_SETTINGS']['FIELDS_MAP'] = $fieldsList;

		//$arProp['VALUES'] = RSDevFuncOffersExtension::GetSortedPropertiesValues($arProp);

		static $PROP_CACHE = array();

		if (isset($PROP_CACHE[$arProp['ID']])) {
			$arProp['VALUES'] = $PROP_CACHE[$arProp['ID']];
		} else {
			$arProp['VALUES'] = array();
			...
			while ($oneEnum = $propEnums->fetch())
			{
				...
				$arProp['VALUES'][$oneEnum['ID']] = array_combine(
					array_map(
						function($sKey){
							return substr($sKey, 0, 3) == 'UF_' ? substr_replace($sKey, '' , 0, 3) : $sKey;
						},
						array_keys($oneEnum)
					),
					$oneEnum
			...
			if (!empty($arProp['VALUES'])) {
				$PROP_CACHE[$arProp['ID']] = $arProp['VALUES'];
			}
		}
	}
}
