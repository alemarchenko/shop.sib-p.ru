...
if($this->StartResultCache(false, 'v9'.($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	if(!empty($arResult["ITEMS"]))
	{
		if ($this->facet->isValid())
		{
            ...
            if(isset($arParams['CUSTOM_ELEMENT_FILTER']))
            {
                if(is_array($arParams['CUSTOM_ELEMENT_FILTER']))
                {
                    $arResult['FACET_FILTER'] = array_merge($arResult['FACET_FILTER'], $arParams['CUSTOM_ELEMENT_FILTER']);
                }
                elseif(is_string($arParams['CUSTOM_ELEMENT_FILTER']) && isset($GLOBALS[$arParams['CUSTOM_ELEMENT_FILTER']]))
                {
                    $arResult['FACET_FILTER'] = array_merge($arResult['FACET_FILTER'], $GLOBALS[$arParams['CUSTOM_ELEMENT_FILTER']]);
                }
            }
		...
		else
		{
            ...
            if(isset($arParams['CUSTOM_ELEMENT_FILTER']))
            {
                if(is_array($arParams['CUSTOM_ELEMENT_FILTER']))
                {
                    $arElementFilter = array_merge($arElementFilter, $arParams['CUSTOM_ELEMENT_FILTER']);
                }
                elseif(is_string($arParams['CUSTOM_ELEMENT_FILTER']) && isset($GLOBALS[$arParams['CUSTOM_ELEMENT_FILTER']]))
                {
                    $arElementFilter = array_merge($arElementFilter, $GLOBALS[$arParams['CUSTOM_ELEMENT_FILTER']]);
                }
            }
