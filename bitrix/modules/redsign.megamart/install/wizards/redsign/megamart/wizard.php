...
class SelectSiteStep extends CSelectSiteWizardStep
{
	function InitStep()
	{
		...
		$wizard->solutionName = "redsign.megamart";
	}
}
...
class SelectTemplateStep extends CSelectTemplateWizardStep
{
	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			...
			if (in_array($templateID,  array("megamart")))
				$wizard->SetVar("templateID", "megamart");
		}
	}
	...
	function ShowStep()
	{
		...
		if (in_array("megamart", array_keys($arTemplates)))
		{
			$arTemplateOrder[] = "megamart";
		}

		$defaultTemplateID = COption::GetOptionString("main", "wizard_template_id", "megamart", $wizard->GetVar("siteID"));
		if (!in_array($defaultTemplateID, array("megamart"))) $defaultTemplateID = "megamart";
		...
		$arTemplateInfo = array(
			"megamart" => array(
				...
				"DESCRIPTION" => "",
				"PREVIEW" => $wizard->GetPath()."/site/templates/megamart/preview.gif",
				"SCREENSHOT" => $wizard->GetPath()."/site/templates/megamart/screen.gif",
			),
		);

	//	$this->content .= "<input type='hidden' value='redsign.megamart' name='templateID' id='templateID'>";//$this->ShowInputField('hidden', 'templateID', array("id" => "templateID", "value" => "megamart"));

		if (!class_exists('CUpdateClient'))
		{
			require_once(Bitrix\Main\Application::getDocumentRoot().'/bitrix/modules/main/classes/general/update_client.php');
		}

		$bitrixKey = \CUpdateClient::GetLicenseKey();
		$bitrixKeyHash =  md5('BITRIX'.$bitrixKey.'LICENCE');

		$this->content .= '<img src="https://static.redsign.ru/redsign/logo.png?'.$bitrixKeyHash.'_'.\Bitrix\Main\Security\Random::getString(12).'" style="position: absolute; right: 0; top: 0; width: 1px; height: 1px;">';
...
class SiteSettingsStep extends CSiteSettingsWizardStep
{
	function InitStep()
	{
		...
		$isWizardInstalled = COption::GetOptionString("redsign.megamart", "wizard_installed", "N", $siteID) == "Y";

		if(
			(
				COption::GetOptionString("redsign.megamart", "wizard_installed", "N", $siteID) == "Y"
		...
		$wizard->SetDefaultVars(Array(
			"siteLogo" => file_exists(WIZARD_SITE_PATH."include/logo.png") ? WIZARD_SITE_DIR."include/logo.png" : ($isWizardInstalled ? "" : "/bitrix/wizards/redsign/megamart/site/templates/megamart/assets/img/logo.png"),
			"siteLogoRetina" => file_exists(WIZARD_SITE_PATH."include/logo_retina.png") ? WIZARD_SITE_DIR."include/logo_retina.png" : ($isWizardInstalled ? "" : "/bitrix/wizards/redsign/megamart/site/templates/megamart/assets/img/logo.png"),
/*
			"siteLogoMobile" => file_exists(WIZARD_SITE_PATH."include/logo_mobile.png") ? WIZARD_SITE_DIR."include/logo_mobile.png" : ($isWizardInstalled ? "" : "/bitrix/wizards/redsign/megamart/site/templates/megamart/themes/".$themeID."/images/logo_mobile.png"),
			"siteLogoMobileRetina" => file_exists(WIZARD_SITE_PATH."include/logo_mobile_retina.png") ? WIZARD_SITE_DIR."include/logo_mobile_retina.png" : ($isWizardInstalled ? "" : "/bitrix/wizards/redsign/megamart/site/templates/megamart/themes/".$themeID."/images/logo_mobile_retina.png")
*/
		));
		...
		$wizard->SetDefaultVars(
			Array(
				...
				"shopEmail" => COption::GetOptionString("redsign.megamart", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				'mapService' => COption::GetOptionString("redsign.megamart", "mapService", 'google', $siteID),
				"siteMetaDescription" => GetMessage("wiz_site_desc"),
				"siteMetaKeywords" => GetMessage("wiz_keywords"),
				// "installEshopApp" => COption::GetOptionString("redsign.megamart", "installEshopApp", "Y", $siteID),
			)
		);
...
*/
		...
		$this->content .=
			'<div class="wizard-input-title">'.GetMessage("WIZ_MAP_SERVICE").'</div>
			<div class="wizard-input-form-block" >'.
				$this->ShowSelectField("mapService", array(
					"yandex" => GetMessage("WIZ_MAP_SERVICE_YANDEX"),
					"google" => GetMessage("WIZ_MAP_SERVICE_GOOGLE"),
				), array("class" => "wizard-field")).'
			</div>';
...
class CatalogSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"catalogSubscribe" => (isset($arSubscribe[$siteID])) ? ($arSubscribe[$siteID]['use'] == "Y" ? "Y" : false) : "Y",//COption::GetOptionString("redsign.megamart", "catalogSubscribe", "Y", $siteID),
				"catalogView" => COption::GetOptionString("redsign.megamart", "catalogView", "list", $siteID),
...
class ShopSettings extends CWizardStep
{
	function InitStep()
	{
		...
		$wizard->SetDefaultVars(
			Array(
				"shopLocalization" => COption::GetOptionString("redsign.megamart", "shopLocalization", "ru", $siteID),
				"shopEmail" => COption::GetOptionString("redsign.megamart", "shopEmail", "sale@".$_SERVER["SERVER_NAME"], $siteID),
				"shopOfName" => COption::GetOptionString("redsign.megamart", "shopOfName", GetMessage("WIZ_SHOP_OF_NAME_DEF"), $siteID),
				"shopLocation" => COption::GetOptionString("redsign.megamart", "shopLocation", GetMessage("WIZ_SHOP_LOCATION_DEF"), $siteID),
				//"shopZip" => 101000,
				"shopAdr" => COption::GetOptionString("redsign.megamart", "shopAdr", GetMessage("WIZ_SHOP_ADR_DEF"), $siteID),
				"shopINN" => COption::GetOptionString("redsign.megamart", "shopINN", "1234567890", $siteID),
				"shopKPP" => COption::GetOptionString("redsign.megamart", "shopKPP", "123456789", $siteID),
				"shopNS" => COption::GetOptionString("redsign.megamart", "shopNS", "0000 0000 0000 0000 0000", $siteID),
				"shopBANK" => COption::GetOptionString("redsign.megamart", "shopBANK", GetMessage("WIZ_SHOP_BANK_DEF"), $siteID),
				"shopBANKREKV" => COption::GetOptionString("redsign.megamart", "shopBANKREKV", GetMessage("WIZ_SHOP_BANKREKV_DEF"), $siteID),
				"shopKS" => COption::GetOptionString("redsign.megamart", "shopKS", "30101 810 4 0000 0000225", $siteID),
				"siteStamp" => COption::GetOptionString("redsign.megamart", "siteStamp", $siteStamp, $siteID),

				//"shopCompany_ua" => COption::GetOptionString("redsign.megamart", "shopCompany_ua", "", $siteID),
				"shopOfName_ua" => COption::GetOptionString("redsign.megamart", "shopOfName_ua", GetMessage("WIZ_SHOP_OF_NAME_DEF_UA"), $siteID),
				"shopLocation_ua" => COption::GetOptionString("redsign.megamart", "shopLocation_ua", GetMessage("WIZ_SHOP_LOCATION_DEF_UA"), $siteID),
				"shopAdr_ua" => COption::GetOptionString("redsign.megamart", "shopAdr_ua", GetMessage("WIZ_SHOP_ADR_DEF_UA"), $siteID),
				"shopEGRPU_ua" =>  COption::GetOptionString("redsign.megamart", "shopEGRPU_ua", "", $siteID),
				"shopINN_ua" =>  COption::GetOptionString("redsign.megamart", "shopINN_ua", "", $siteID),
				"shopNDS_ua" =>  COption::GetOptionString("redsign.megamart", "shopNDS_ua", "", $siteID),
				"shopNS_ua" =>  COption::GetOptionString("redsign.megamart", "shopNS_ua", "", $siteID),
				"shopBank_ua" =>  COption::GetOptionString("redsign.megamart", "shopBank_ua", "", $siteID),
				"shopMFO_ua" =>  COption::GetOptionString("redsign.megamart", "shopMFO_ua", "", $siteID),
				"shopPlace_ua" =>  COption::GetOptionString("redsign.megamart", "shopPlace_ua", "", $siteID),
				"shopFIO_ua" =>  COption::GetOptionString("redsign.megamart", "shopFIO_ua", "", $siteID),
				"shopTax_ua" =>  COption::GetOptionString("redsign.megamart", "shopTax_ua", "", $siteID),

				"installPriceBASE" => COption::GetOptionString("redsign.megamart", "installPriceBASE", "Y", $siteID),
			)
		);
	}
	...
	function InitStep()
	{
		...
		if ($shopLocalization == "ua")
			...
			$wizard->SetDefaultVars(
				Array(
					"personType" => Array(
						"fiz" =>  COption::GetOptionString("redsign.megamart", "personTypeFiz", "Y", $siteID),
						"ur" => COption::GetOptionString("redsign.megamart", "personTypeUr", "Y", $siteID),
					)
				)
			);
	}
...
===*/
	...
	function ShowStep()
	{
		...
		)
		{
			...
			if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", $siteID) != "Y")
			{
			...
			{
				$deliveryContent ='
					<link rel="stylesheet" type="text/css" href="/bitrix/wizards/redsign/megamart/css/style.css">
...
class FinishStep extends CFinishWizardStep
{
	...
	//	$this->content .= "<br clear=\"all\"><a href=\"/bitrix/admin/wizard_install.php?lang=".LANGUAGE_ID."&site_id=".$siteID."&wizardName=redsign:megamart.mobile&".bitrix_sessid_get()."\" class=\"button-next\"><span id=\"next-button-caption\">".GetMessage("wizard_store_mobile")."</span></a><br>";
