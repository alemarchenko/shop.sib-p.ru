...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_system_size_tables_".WIZARD_SITE_ID;
		$iblockType = "system";
		...
		{
            // CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/company/index.php", array("SYSTEM_SIZE_TABLES_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array('en', 'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/size_tables-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/size_tables-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_system_size_tables",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
...
WizardServices::IncludeServiceLang("system_size_tables.php", $lang);
...
if (count($arReplace) > 0) {
    CUserOptions::SetOption(
        ...
        array(
        'tabs' => GetMessage('IBLOCK_TAB_SETTINGS', $arReplace),
        )
    );
}
