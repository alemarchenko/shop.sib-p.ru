...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
			...
			else
			{
				BX.ajax({
					...
					onsuccess: BX.delegate(function(result) {
						...
						/* CUSTOM */
						$(this.orderBlockNode).removeData('bmd.bootstrapMaterialDesign');
						RS.Init(['bmd'], this.orderBlockNode);
						/* /CUSTOM */
		...
		bindEvents: function()
		{
			...
			/* CUSTOM */
			$('.js-compact-header').on('fixed.compact-header',  BX.proxy(function () {
		...
		editRegistrationForm: function(authContent)
		{
			...
			else
			{
				...
				{
					authFormNodes.push(BX.create('DIV', {
						props: {className: 'bx-authform-formgroup-container captcha-wrap'},
						children: [
							BX.create('DIV', {
...
*/
								...
								BX.create('LABEL', {
									attrs: {
										className: "radio-inline bmd-custom-radio-inline",
...
*/
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-item-container'}}),
				paySystemItemNode, i, innerPs;

			if (this.result.PAY_FROM_ACCOUNT == 'Y')
			{
				innerPs = this.getInnerPaySystem();
				paySystemItemsContainer.appendChild(innerPs);
			}
		...
		createPaySystemItem: function(item)
		{
			...
			label = BX.create('DIV', {
				...
				children: [
					...
					BX.create('DIV', {
						props: {
							className: 'checkbox bmd-custom-checkbox'
						},
						children: [
							BX.create('label', {
								children: [
			...
			description = BX.create('DIV', {
				props: {className: 'd-block pl-6'},
			})
			this.editPaySystemInfo(description, item);
		...
		editPaySystemInfo: function(paySystemNode, currentPaySystem)
		{
			...
			// if (this.result.PAY_FROM_ACCOUNT == 'Y' && currentPaySystem.CHECKED == 'Y')
			// 	innerPs = this.getInnerPaySystem();

			if (currentPaySystem)
			{
				if (!!currentPaySystem.DESCRIPTION)
				{
					title = BX.create('DIV', {
		...
		getInnerPaySystem: function()
		{
			...
			innerPsDesc = BX.create('DIV', {props: {className: 'd-block pl-6 bx-soa-pp-company-desc'}, html: htmlString});
		...
		createDeliveryItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				...
				logotype, label, title, itemNode, logoNode, bmdInput, description;


			label = BX.create('DIV', {
				props: {
					className: 'd-block '
					+ (item.CALCULATE_ERRORS || deliveryCached && deliveryCached.CALCULATE_ERRORS ? ' bx-bd-warning' : '')},
				children: labelNodes
			});
			...
			this.editDeliveryInfo(description, item);
		...
		isValidProperty: function(data, fieldName)
		{
			...
			{
				if (this.readyValidationProperties[data.inputs[i].id])
				{
			...
			if (data && data.inputs && data.action)
			{
				for (i = 0; i < data.inputs.length; i++)
				{
					if (BX.type.isElementNode(data.inputs[i]))
						BX.bind(data.inputs[i], data.action, BX.delegate(function(e){
							this.readyValidationProperties[e.target.id] = true;
							this.isValidProperty(data);
						}, this));
					else
		...
		showErrorTooltip: function(tooltipId, targetNode, text)
		{
			...
			var tooltip = BX('tooltip-' + tooltipId),
				tooltipInner, quickLocation, input;

			text = this.uniqueText(text, '<br>');

			input = targetNode.parentNode.querySelector('input') || targetNode.parentNode.querySelector('textarea');
			...
			else
			{
				...
				input = targetNode.parentNode.querySelector('input') || targetNode.parentNode.querySelector('textarea');
				if (input) {
					BX.insertAfter(tooltip, targetNode.nextSibling);
				} else {
		...
		closeErrorTooltip: function(tooltipId)
		{
			...
			if (tooltip)
			{
				...
				// CUSTOM
				var input = tooltip.parentNode.querySelector('input') || tooltip.parentNode.querySelector('textarea');
				if (
					input &&
		...
		editTotalBlock: function()
		{
			...
			var totalInfoBlockHeader = document.createElement('div');
			totalInfoBlockHeader.innerHTML = '<div class="bx-soa-cart-total-heading"><h5 class="mb-0">' + BX.message('SOA_TOTAL_YOUR_ORDER') + '</h5><a class="small" href="' + this.params['PATH_TO_BASKET'] + '">' +  BX.message('SOA_TOTAL_CHANGE_ORDER')  + '</a></div>';

			this.totalInfoBlockNode.appendChild(totalInfoBlockHeader);
			...
			if (!this.result.SHOW_AUTH)
			{
				var allowOrder = this.result.ALLOW_ORDER !== undefined ? this.result.ALLOW_ORDER : true,
					checkoutButton = this.orderSaveBlockNode.querySelector('[data-save-button]');
						...
						children: [
							BX.create('A', {
								props: {
									...
									className: 'btn btn-primary btn-lg btn-order-save'+ (allowOrder ? '' : ' disabled'),
									disabled: allowOrder ?  '' : 'disabled',
								},
		...
		totalBlockScrollCheck: function()
		{
			...
			var fixedHeader = document.querySelector('.js-compact-header.is-fixed .l-compact-header');
			var offsetTop = 0;
			if(fixedHeader) {
				offsetTop = fixedHeader.clientHeight + 10;
			}

			if (scrollTop + offsetTop > ghostTop && !BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
			{
				...
				this.totalInfoBlockNode.style.top = offsetTop + 'px';
			}
			else if (scrollTop + offsetTop < ghostTop && BX.hasClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed'))
			{
				BX.removeClass(this.totalInfoBlockNode, 'bx-soa-cart-total-fixed');
