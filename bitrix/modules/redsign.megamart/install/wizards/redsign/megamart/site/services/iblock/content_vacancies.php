...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_content_vacancies_".WIZARD_SITE_ID;
		$iblockType = "content";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/company/vacancies/index.php", array("CONTENT_VACANCIES_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/vacancies-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/vacancies-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_content_vacancies",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
...
WizardServices::IncludeServiceLang("content_vacancies.php", $lang);
