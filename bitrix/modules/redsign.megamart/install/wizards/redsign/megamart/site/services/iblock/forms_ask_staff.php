...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_forms_ask_staff_".WIZARD_SITE_ID;
		$iblockType = "forms";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/forms/ask_staff/index.php", array("FORMS_ASK_STAFF_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/ask_staff-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/ask_staff-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_forms_ask_staff",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
...
WizardServices::IncludeServiceLang("forms_ask_staff.php", $lang);
