...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "redsign_megamart_banner_outer_sidebar_".WIZARD_SITE_ID;
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/sect_sidebar_outer.php", array("ADV_BANNERS_OUTER_SIDEBAR_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/outer_sidebar-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/outer_sidebar-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_banner_outer_sidebar",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
	...
	$arFields = Array(
		...
		"CODE" => "redsign_megamart_banner_outer_sidebar",
		"XML_ID" => $iblockCode,
