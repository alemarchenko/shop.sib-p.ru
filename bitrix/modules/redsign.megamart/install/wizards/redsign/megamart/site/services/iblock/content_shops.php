...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_content_shops_".WIZARD_SITE_ID;
		$iblockType = "content";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/contacts/shops/index.php", array("CONTENT_SHOPS_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/index/shops.php", array("CONTENT_SHOPS_IBLOCK_ID" => $iblockID));
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/header/search_popup.php", array("CONTENT_SHOPS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/shops-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/shops-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_content_shops",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
...
if ($iblockID)
{
	$sMapService = $wizard->GetVar("mapService");
	$sPropType = '';

	switch ($sMapService)
	{
		case 'yandex':
			$sPropType = 'map_yandex';
			break;
		case 'google':
		default:
			$sPropType = 'map_google';
			break;
	}
...
WizardServices::IncludeServiceLang("content_shops.php", $lang);
