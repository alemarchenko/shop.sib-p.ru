...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_banner_mozaic_side_".WIZARD_SITE_ID;
		$iblockType = "banners";
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/index/mozaic_banner_1.php", array("BANNERS_MOZAIC_SIDE_IBLOCK_ID" => $iblockID));
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/index/mozaic_banner_2.php", array("BANNERS_MOZAIC_SIDE_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/mozaic_side-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/mozaic_side-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_banner_mozaic_side",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
