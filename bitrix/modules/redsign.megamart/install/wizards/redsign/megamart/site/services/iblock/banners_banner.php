...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_banners_banner_".WIZARD_SITE_ID;
		$iblockType = "banners";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/header/banner.php", array("BANNERS_BANNER_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/banner-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/banner-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_banners_banner",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
...
WizardServices::IncludeServiceLang("banners_banner.php", $lang);
...
$arPropsToDelete = array("LINK", "BUTTONS");
$arOrder = array();
$arFilter = array(
	"IBLOCK_ID" => $iblockID,
);
		...
		if (!empty($arElement['PROPERTIES'][$code]['VALUE']))
		{
			...
			if (is_array($arElement['PROPERTIES'][$code]['VALUE']))
			{
				foreach ($arElement['PROPERTIES'][$code]['VALUE'] as $key => $sPropValue)
				{
					...
					{
						$propNewValue[$key] = array(
							'VALUE' => str_replace('#SITE_DIR#', '/', $sPropValue),
							'DESCRIPTION' => $arElement['PROPERTIES'][$code]['DESCRIPTION'][$key]
						);
					}
					else
					{
						$propNewValue[$key] = str_replace('#SITE_DIR#', '/', $sPropValue);
					}
				}
			}
			else
			{
				$propNewValue = str_replace('#SITE_DIR#', '/', $arElement['PROPERTIES'][$code]['VALUE']);
			}
