...
if (in_array(LANGUAGE_ID, array('en', 'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/catalog/catalog-sale-'.LANGUAGE_ID.'.xml';
	$iblockXMLFilePrices = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/catalog/catalog-sale-prices-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/catalog/catalog-sale-ru.xml';
	$iblockXMLFilePrices = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/catalog/catalog-sale-prices-ru.xml';
}
...
if(!($dbResultList->Fetch()))
{
	...
	{
		WizardServices::IncludeServiceLang("catalog_catalog.php", $arLanguage["ID"]);
...
if($IBLOCK_CATALOG_ID == false)
{
	...
	$_SESSION["WIZARD_CATALOG_CATALOG_IBLOCK_ID"] = $IBLOCK_CATALOG_ID;
}
else
{
	$arSites = array();
