...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "redsign_megamart_banner_index_mini_".WIZARD_SITE_ID;
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/index/adv_banner_grid.php", array("ADV_BANNERS_INDEX_MINI_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/index_mini-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/index_mini-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_banner_index_mini",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
	...
	$arFields = Array(
		...
		"CODE" => "redsign_megamart_banner_index_mini",
		"XML_ID" => $iblockCode,
