...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_forms_recall_".WIZARD_SITE_ID;
		$iblockType = "forms";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/forms/recall/index.php", array("FORMS_RECALL_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/recall-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/recall-ru.xml';
}
...
WizardServices::IncludeServiceLang("forms_recall.php", $lang);
