...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_catalog_services_".WIZARD_SITE_ID;
		$iblockType = "catalog";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/templates/articles/services_items.php", array("CATALOG_SERVICES_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/templates/news/services_items.php", array("CATALOG_SERVICES_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/include/templates/sale/services_items.php", array("CATALOG_SERVICES_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/services/.mainsub.menu_ext.php", array("CATALOG_SERVICES_IBLOCK_ID" => $iblockID));
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/services/index.php", array("CATALOG_SERVICES_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}
...
if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/services-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/services-ru.xml';
}
...
if($iblockID == false)
{
	...
	$iblockID = WizardServices::ImportIBlockFromXML(
		$iblockXMLFile,
		"redsign_megamart_catalog_services",
		$iblockType,
		WIZARD_SITE_ID,
		$permissions
	);
...
WizardServices::IncludeServiceLang("catalog_services.php", $lang);
...
$arUserFields = array("UF_BROWSER_TITLE", "UF_KEYWORDS", "UF_META_DESCRIPTION", 'UF_BACKGROUND_IMAGE', 'UF_BANNER_TYPE', 'UF_FEATURED_FILTER');
