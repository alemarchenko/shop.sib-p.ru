...
if(COption::GetOptionString("redsign.megamart", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA)
{
	...
	{
		$iblockCode = "redsign_megamart_forms_job_".WIZARD_SITE_ID;
		$iblockType = "forms";
		...
		{
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/forms/product-ask/index.php", array("FORMS_JOB_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockCode = "redsign_megamart_forms_product_ask_".WIZARD_SITE_ID;
$iblockType = "forms";

if (in_array(LANGUAGE_ID, array(/*'en', */'ru'))) {
    $iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/job-'.LANGUAGE_ID.'.xml';
} else {
	$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/'.$iblockType.'/job-ru.xml';
}
...
WizardServices::IncludeServiceLang("forms_job.php", $lang);
...
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/forms/job/index.php", array("FORMS_JOB_IBLOCK_ID" => $iblockID));
?>