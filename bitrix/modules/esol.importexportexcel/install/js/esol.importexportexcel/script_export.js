...
function KdaEEFilter(listIndex, prefix)
{
	...
	{
		var obj = this;
		this.filterBlock = $('#kda-ee-cfilter-'+this.listIndex);
		...
		eval('var filter = '+$('input[name="OLD_FILTER"]', this.filterBlock).val());
	...
	this.AddField = function(filterData)
	{
		var fieldPrefix = (this.prefix ? this.prefix : 'SETTINGS[FILTER]')+'['+this.listIndex+']';
		var field = new KdaEEFilterField(this.filterBlock, fieldPrefix, this.MaxFieldIndex++, filterData);
...
function KdaEEFilterField(filterBlock, fieldPrefix, fieldIndex, filterData)
{
	this.Init = function(filterBlock, fieldPrefix, fieldIndex, filterData)
	{
		this.fieldIndex = fieldIndex;
		this.fieldPrefix = fieldPrefix+'['+this.fieldIndex+']';
		this.filterBlock = filterBlock;
		this.filterType = this.filterBlock.attr('data-type');
		this.block = $('<div class="kda-ee-cfilter-field"></div>');
		...
		var select = $('select[name="S_FIELD"]', this.filterBlock).clone();
		select.removeAttr('id').attr('name', this.fieldPrefix+'[FIELD]');
		new Select2Text(this.fieldBlock, select);
		...
		if(typeof filterData=='object')
		{
			...
			{
				this.SetFieldVal('[name="'+this.fieldPrefix+'['+i+']'+(typeof filterData[i]=='object' ? '[]' : '')+'"]', this.block, filterData[i], 5000);
				//$('[name="'+this.fieldPrefix+'['+i+']"]', this.block).val(filterData[i]).trigger('chosen:updated').trigger('change');
			}
		}
	};
	...
	{
		$('input, select', this.valueBlock).bind('change', function(){
	...
	this.Init(filterBlock, fieldPrefix, fieldIndex, filterData);
}
