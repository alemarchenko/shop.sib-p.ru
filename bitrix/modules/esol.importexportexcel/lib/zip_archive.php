				...
				{					
					...
					if($rowNum > 1)
					{
						...
						if($rowNum > $lastRow)
						{
							$time0 = time();
							$maxTime = 5;
							while($xml->read() && ($xml->nodeType != \XMLReader::ELEMENT || $xml->depth > 1) && time()-$time0 < $maxTime)
							{
								if($xml->nodeType == \XMLReader::ELEMENT && $xml->name=='row' && $xml->depth==2){$rowNum++;}
							}
