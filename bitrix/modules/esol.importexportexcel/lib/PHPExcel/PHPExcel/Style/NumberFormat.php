...
/**
 * KDAPHPExcel_Style_NumberFormat
 *
 * @category   KDAPHPExcel
 * @package	KDAPHPExcel_Style
 * @copyright  Copyright (c) 2006 - 2013 KDAPHPExcel (http://www.codeplex.com/KDAPHPExcel)
 */
class KDAPHPExcel_Style_NumberFormat extends KDAPHPExcel_Style_Supervisor implements KDAPHPExcel_IComparable
{
	/* Pre-defined formats */
	...
	const FORMAT_DATE_XLSX14				= 'dd.mm.yyyy';
	 ...
	 * @var string
	 */
	protected $_formatCode	=	KDAPHPExcel_Style_NumberFormat::FORMAT_GENERAL;
	...
	/**
	 * Create a new KDAPHPExcel_Style_NumberFormat
	 *
	 ...
	 *
	 * @return KDAPHPExcel_Style_NumberFormat
	 */
	 ...
	 *
	 * <code>
	 * $objKDAPHPExcel->getActiveSheet()->getStyle('B2')->getNumberFormat()->applyFromArray(
	 *		array(
	 *			'code' => KDAPHPExcel_Style_NumberFormat::FORMAT_CURRENCY_EUR_SIMPLE
	 *		)
	 * );
	 * </code>
	 *
	 ...
	 * @return KDAPHPExcel_Style_NumberFormat
	 */
	public function setFormatCode($pValue = KDAPHPExcel_Style_NumberFormat::FORMAT_GENERAL)
	{
		if ($pValue == '') {
			$pValue = KDAPHPExcel_Style_NumberFormat::FORMAT_GENERAL;
		}
		if ($this->_isSupervisor) {
	...
	private static function fillBuiltInFormatCodes()
	{
		...
		if (is_null(self::$_builtInFormats)) {
			...
			// General
			self::$_builtInFormats[0] = KDAPHPExcel_Style_NumberFormat::FORMAT_GENERAL;
			...
			self::$_builtInFormats[22] = 'dd.mm.yyyy h:mm';
	...
	private static function _formatAsDate(&$value, &$format)
	{
		...
		$dateObj = KDAPHPExcel_Shared_Date::ExcelToPHPObject($value);
	...
	private static function _formatAsFraction(&$value, &$format)
	{
		...
		$GCD = KDAPHPExcel_Calculation_MathTrig::GCD($decimalPart,$decimalDivisor);
	 ...
	 */
	public static function toFormattedString($value = '0', $format = KDAPHPExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null)
	{
		...
		if (($format === KDAPHPExcel_Style_NumberFormat::FORMAT_GENERAL) || ($format === KDAPHPExcel_Style_NumberFormat::FORMAT_TEXT)) {
			return self::getFinalNumber($value);
		}
		...
		if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) { // datetime format
			...
			if ($format === self::FORMAT_CURRENCY_EUR_SIMPLE) {
				...
				if(preg_match_all('/"[^"]*"/', $format, $m))
				{
					...
					foreach($m[0] as $v)
					{
						$v1 = substr($format, 0, strpos($format, $v));
						$format = substr($format, strlen($v1)+strlen($v));
						if(strlen($v1) > 0) $newValue .= self::toFormattedString($value, $v1, $callBack);
						$newValue .= trim($v, '"');
					}
					if(strlen($format) > 0) $newValue .= self::toFormattedString($value, $format, $callBack);
					return $newValue;
				}
				/*/Fix quotes*/
				...
				if (preg_match('/#?.*\?\/\?/', $format, $m)) {
					...
					if(preg_match('/^((#+\s*)+0+)(\.0+)?$/', $format, $m))
					{
						$v1 = $m[1];
						$v2l = strlen(current(explode('.', $value)));
						while(strpos($v1, '#')===0 && strlen(str_replace(' ', '', $v1)) > $v2l)
						{
							$v1 = trim(substr($v1, 1));
						}
						if($v1!=$m[1]) $format = $v1.$m[3];
					}

					// Strip #
					$format = preg_replace('/\\#/', '0', $format);
					$format = preg_replace('/(\x82\xBD)+/s', '', $format);
					...
					if (preg_match($number_regex, $m, $matches)) {
						...
						if ($useThousands) {
							$value = number_format(
										$value
										, strlen($right)
										, KDAPHPExcel_Shared_String::getDecimalSeparator()
										, KDAPHPExcel_Shared_String::getThousandsSeparator()
									);
				...
				if (preg_match('/\[\$(.*)\]/u', $format, $m)) {
					...
					if(!preg_match('/\[\$(.*)\]/u',$value) 
						&& mb_strpos($format, KDAPHPExcel_Style_NumberFormat::FORMAT_GENERAL)===false
						&& mb_strpos($format, '/')===false) $value = $format;
					$currencyFormat = $m[0];
					$currencyCode = $m[1];
					list($currencyCode) = explode('-',$currencyCode);
					if ($currencyCode == '') {
						$currencyCode = KDAPHPExcel_Shared_String::getCurrencyCode();
					}
	...
	public static function getFinalNumber($value)
	{
		$oProfile = \CKDAImportProfile::getInstance();
		if($oProfile->GetParam('NUMBER_SEPARATOR')==',') $value = str_replace('.', ',', $value);
		return $value;
	}
}
