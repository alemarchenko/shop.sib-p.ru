...
class KDAPHPExcel_Reader_Excel5 extends KDAPHPExcel_Reader_Abstract implements KDAPHPExcel_Reader_IReader
{
	...
	public function load($pFilename)
	{
		...
		foreach ($this->_sheets as $sheet) {
			...
			$loop = 0;
			$firstPos = $this->_pos;
			//while for read merged cells
			while($loop < 1 || ($bmc && $loop < 2))
			{
				$loop++;
				$this->_pos = $firstPos;
				...
				while ($this->_pos <= $this->_dataSize - 4) {
					...
					switch ($code) {
						...
						case self::XLS_Type_MERGEDCELLS:			$this->_readMergedCells();$bmc=true;break;
...
//				echo '<br />';
				...
				if (isset($allSpContainers[$n + 1]) && is_object($allSpContainers[$n + 1])) {
					...
					{
						if($obj['otObjType']==0x08 && $this->getReadFilter()->getParam('ELEMENT_LOAD_IMAGES')!='Y') //image
						{
							continue;
						}
						elseif(!$this->getReadFilter()->readCell($startColumn, $startRow, $this->_phpSheet->getTitle()))
						{
							continue;
						}
					}
...
//							echo 'Picture Object<br />';
							...
							if(is_callable(array($escherWorkbook->getDggContainer()->getBstoreContainer(), 'getBSECollection')))
							{
								$BSECollection = $escherWorkbook->getDggContainer()->getBstoreContainer()->getBSECollection();
							}
							else $BSECollection = array();
							...
							if(isset($BSECollection[$BSEindex - 1]))
							{
								...
								if ($blip = $BSE->getBlip()) {
									...
									$drawing->setCoordinates($spContainer->getStartCoordinates(), $spContainer->getEndCoordinates());
								}
							}
...
//					echo '<br />';
		...
		if($this->_phpExcel->getSheetCount()==0)
		{
...
//		$recordData = cp_substr($this->_data, $this->_pos + 4, $length);
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if(array_key_exists($fontIndex, $this->_objFonts))
	...
	private function _readSheet()
	{
		...
		$this->_sheets[] = array(
			'name' => (strlen($rec_name) > 0 ? $rec_name : 'idx'.(count($this->_sheets) + 1)),
			'offset' => $rec_offset,
	...
	private function _readNumber()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// add cell value
			$cell->setValueExplicit($numValue, (is_string($numValue) ? KDAPHPExcel_Cell_DataType::TYPE_STRING : KDAPHPExcel_Cell_DataType::TYPE_NUMERIC));
		}
	}
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			foreach ($cellRangeAddressList['cellRangeAddresses'] as $cellRangeAddress) {
				if ((strpos($cellRangeAddress,':') !== FALSE) &&
					...
					if ($this->getReadFilter() !== NULL)
					{
						$this->getReadFilter()->setMergeCells($cellRangeAddress);
					}
				}
			}
		}
...
//			var_dump($bcBitCount);
	 ...
	 * @return float
	 */
	private static function _extractNumber($data, $precision=0)
	{
		if($precision > 0)
		{
		...
		if($precision==0 &&  ($value / pow(10, 14) > 1))
		{
			$pr = 14;
			while($value / pow(10, $pr) > 1) $pr++;
			return self::_extractNumber($data, $pr);
		}
		if($precision > 0)
		{
			$value = (string)$value;
