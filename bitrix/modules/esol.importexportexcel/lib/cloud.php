...
class Cloud
{
	...
	public function GdriveGetAccessToken(&$arFile, $id, $type='file')
	{
		...
		if($type=='folder') $apiPath = 'https://www.googleapis.com/drive/v3/files/?q="'.$id.'"+in+parents&fields=files(id,name)&includeItemsFromAllDrives=true&supportsAllDrives=true';
	...
	public function Bitrix24GetFile(&$tmpPath, $path, $fromFile=false)
	{
		...
		$res = $client->get($path);
		$arCookies = $client->getCookies()->toArray();
		if(preg_match('/<a[^>]+href="([^"]*downloadFolderArchive[^"]*)"/Uis', $res, $m))
		{
			$loc = $m[1];
