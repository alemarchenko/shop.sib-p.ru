...
class Imap
{
	...
	protected function select($mailbox, &$error)
	{
		...
		$response = $this->executeCommand(sprintf(
			'SELECT "%s"', /*$this->encodeUtf7Imap($mailbox)*/ $mailbox
		), $error);
	 ...
	 * @return int|false
	 */
	public function getSearch($mailbox, $arCriterias, &$error)
	{
		$error = null;
		...
		$command = 'SEARCH CHARSET '.$this->options['encoding'];
		$endCommand = '';
		...
		if($this->SupportExtCriteria() && $arCriterias['SUBJECT'])
		{
			...
			{
				$command .= sprintf(' SUBJECT "%s"', $arCriterias['SUBJECT']);
			}
			else
			{
				$command .= ' SUBJECT {'.mb_strlen($arCriterias['SUBJECT'], 'CP1251').'}';
				$endCommand .= "\x00".$arCriterias['SUBJECT'];
			}
		}
		...
		if(!$this->SupportExtCriteria() && (strlen($arCriterias['FROM']) > 0 || strlen($arCriterias['SUBJECT']) > 0))
		{
			...
			while($i>=0)
			{
				...
				if($header!==false
					...
					&& (strlen($arCriterias['FILENAME'])==0 || $arCriterias['FILENAME_REGEXP']=='Y' || $this->getMessageFile($mailbox, $uid, $arCriterias, array('txt', 'csv', 'xls', 'xlsx', 'xlsm', 'zip', 'gz', 'tgz', 'rar'), $error)!==false))
				{
					return array($uid);
				}
				$i--;
	...
	public function SupportExtCriteria()
	{
		$support = true;
		if(ToLower($this->options['host'])=='imap.mail.ru')
		{
			$support = false;
		}
		return $support;
	...
	public function getMessage($mailbox, $id, $section, &$error)
	{
		...
		if($section=='HEADER')
		{
			list($header, $htmlBody, $textBody, $parts) = \Bitrix\KdaImportexcel\MailMessage::parseMessage($sbody, $this->options['encoding']);
			return $header;
		}
		...
		if($fullData)
		{
			...
			if(strlen(trim($htmlBody))==0) $htmlBody = \Bitrix\Main\Text\Encoding::convertEncoding(base64_decode($sbody), $this->options['encoding'], \CKDAImportUtils::getSiteEncoding());
			return array(
				'TEXT' => $htmlBody,
	...
	protected function getUntaggedSearch($unset = false)
	{
		$regex = '/^ \* \x20 SEARCH ( (?: \x20 \d+ ){0,10} ) /ix';
		$result = array();
		...
		for ($i = 0; $i < count($this->sessUntagged); $i++)
		{
			...
			$matches[0] = $this->sessUntagged[$i];
			$matches[1] = substr($this->sessUntagged[$i], stripos($this->sessUntagged[$i], 'SEARCH') + 6);
			$result[] = array($this->sessUntagged[$i], $matches);
	...
	public static function decodeError($code)
	{
		switch ($code)
		{
			case self::ERR_CONNECT:
				return Loc::getMessage('KDA_IE_IMAP_ERR_CONNECT');
			case self::ERR_REJECTED:
				return Loc::getMessage('KDA_IE_IMAP_ERR_REJECTED');
			case self::ERR_COMMUNICATE:
				return Loc::getMessage('KDA_IE_IMAP_ERR_COMMUNICATE');
			case self::ERR_EMPTY_RESPONSE:
				return Loc::getMessage('KDA_IE_IMAP_ERR_EMPTY_RESPONSE');
			case self::ERR_BAD_SERVER:
				return Loc::getMessage('KDA_IE_IMAP_ERR_BAD_SERVER');
			case self::ERR_STARTTLS:
				return Loc::getMessage('KDA_IE_IMAP_ERR_STARTTLS');
			case self::ERR_COMMAND_REJECTED:
				return Loc::getMessage('KDA_IE_IMAP_ERR_COMMAND_REJECTED');
			case self::ERR_CAPABILITY:
				return Loc::getMessage('KDA_IE_IMAP_ERR_CAPABILITY');
			case self::ERR_AUTH:
				return Loc::getMessage('KDA_IE_IMAP_ERR_AUTH');
			case self::ERR_AUTH_MECH:
				return Loc::getMessage('KDA_IE_IMAP_ERR_AUTH_MECH');
			case self::ERR_LIST:
				return Loc::getMessage('KDA_IE_IMAP_ERR_LIST');
			case self::ERR_SELECT:
				return Loc::getMessage('KDA_IE_IMAP_ERR_SELECT');
			case self::ERR_SEARCH:
				return Loc::getMessage('KDA_IE_IMAP_ERR_SEARCH');
			case self::ERR_FETCH:
				return Loc::getMessage('KDA_IE_IMAP_ERR_FETCH');
			case self::ERR_APPEND:
				return Loc::getMessage('KDA_IE_IMAP_ERR_APPEND');
			case self::ERR_STORE:
				return Loc::getMessage('KDA_IE_IMAP_ERR_STORE');

			default:
				return Loc::getMessage('KDA_IE_IMAP_ERR_DEFAULT');
		}
	}
}