...
class CKDAExportExcelWriterXlsx {
    ...
    public function openWorkSheet($listIndex)
    {
		...
		$sheetExists = (bool)file_exists($dirPath.'data/xl/worksheets/sheet'.$sheetId.'.xml');
			...
			if(file_exists($tmpSheet))
			{
				...
				if($sxml->sheetData && $sxml->sheetData->row)
				{
					...
					foreach($sxml->sheetData->row as $row)
					{
						...
						if((int)$row->attributes()->r > $this->numRows) $this->numRows = (int)$row->attributes()->r;
					}
					$rows += $this->numRows;
				}
    ...
    public function closeExcelWriter($break = false)
    {
		...
		exec('cd '.$dirPath.'data/; zip -r -'.($this->ee->stepparams['EXPORT_MODE']=='CRON' ? '9' : '5').' "'.$this->outputFile.'" .');
