...
class CKDAExportExcel {
	...
	public function GetExportData($listIndex, $limit=10, $page=1, $sectionKey=1)
	{
		...
		if(!isset($this->filters[$listIndex]))
		{
			...
			if($this->params['FILTER'][$listIndex])
			{
				...
				$arProductIds = false;
				if(\Bitrix\Main\Loader::includeModule('sale') && class_exists('\Bitrix\Sale\Internals\BasketTable'))
				{
					$arOrderFilter = array();
		...
		foreach($arSections as $skey=>$arSection)
		{
			...
			foreach($arElementList as $arElement)
			{
				...
				if(!empty($arFieldsIpropTemp))
				{
					...
					if(count($arFieldsIpropTempCh) > 0) $arFieldsIpropTemp = array_diff($arFieldsIpropTemp, $arFieldsIpropTempCh);
					$arFieldsIpropTempTmpl = preg_grep('/^TEMPLATE_/', $arFieldsIpropTemp);
					if(count($arFieldsIpropTempTmpl) > 0) $arFieldsIpropTemp = array_diff($arFieldsIpropTemp, $arFieldsIpropTempTmpl);
					...
					foreach($arFieldsIpropTemp as $key)
					{
	...
	public function GetSectionIpropTemplates(&$arSection, $arSelectFields)
	{
		...
		if(in_array($sectPathKey, $arSelectFields))
		{
			...
			ksort($arSectionNames, SORT_NUMERIC);
			if(isset($this->customFieldSettings['ISECT_PATH_NAMES']['SECTION_PATH_SEPARATOR']) && strlen($this->customFieldSettings['ISECT_PATH_NAMES']['SECTION_PATH_SEPARATOR']) > 0) $separator = $this->customFieldSettings['ISECT_PATH_NAMES']['SECTION_PATH_SEPARATOR'];
			else $separator = ' / ';
