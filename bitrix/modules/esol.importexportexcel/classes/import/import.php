...
class CKDAImportExcel {
	...
	public function OnFileSaveHandler(&$arFile, $strFileName, $strSavePath, $bForceMD5, $bSkipExt, $dirAdd)
	{
		if($arFile['SAVE_ORIGINAL_PATH']=='Y')
		{
			...
			else $subdir = '/'.$uploadDir.str_repeat('/..', count(explode('/', trim($uploadDir, '/')))).$subdir;
		...
		if($bElemDeactivate || $bSetDefaultProps || $bSetDefaultProps2)
		{
			...
			while($this->stepparams['deactivate_element_first'] < $this->stepparams['deactivate_element_last'])
			{
				...
				foreach($arFieldsList as $key=>$arFields)
				{
					...
					if($this->stepparams['deactivate_element_first2'] > $this->stepparams['deactivate_element_first'] && $this->stepparams['deactivate_element_first2'] > 0) $arFields['>ID'] = $this->stepparams['deactivate_element_first2'];
	...
	public function UpdateElement($ID, $IBLOCK_ID, $arFieldsElement, $arElement=array(), $arElementSections=array(), $isOffer=false)
	{
		...
		{
			if($this->IsChangedElement()) \Bitrix\KdaImportexcel\DataManager\InterhitedpropertyValues::ClearElementValues($IBLOCK_ID, $ID);
	...
	public function PrepareElementPictures(&$arFieldsElement, $isOffer=false)
	{
		...
		foreach($arPictures as $picName)
		{
			if($arFieldsElement[$picName])
			{
				...
				$arFile = $this->GetFileArray($val, array('FILETYPE'=>'IMAGE'), 'IE_'.$picName);
				if(empty($arFile) && strpos($val, $this->params['ELEMENT_MULTIPLE_SEPARATOR'])!==false)
				{
					...
					{
						$arFile = $this->GetFileArray($val, array('FILETYPE'=>'IMAGE'));
					}
				}
	...
	public function SaveSKU($ID, $NAME, $IBLOCK_ID, $arItem, $convertedFields=array())
	{
		...
		$arKeys = array_merge(array('ID', 'NAME', 'IBLOCK_SECTION_ID', 'MODIFIED_BY'), array_keys($arFieldsElement));
		...
		while($arElement = $dbRes->Fetch())
		{
			...
			if($this->SaveElementId($OFFER_ID, 'O'))
			{
				...
				{
					$arProductIds[] = $arElement['PROPERTY_'.$OFFERS_PROPERTY_ID.'_VALUE'];
	...
	public function GetFilterUids($arFieldsElement, $arFieldsElementOrig, $arFieldsProps, $arFieldsPropsOrig, $IBLOCK_ID, $offerPropId=false, $parentId=0)
	{
		...
		foreach($currentUid as $tuid)
		{
			...
			elseif(strpos($tuid, 'IP_PROP')===0)
			{
				...
				elseif($p['PROPERTY_TYPE']=='N' && !is_numeric($valUid) && (!is_array($valUid) || count(preg_grep('/^\d+$/', $valUid))==0))
				{
					$valUid = $valUid2 = '';
				}
				else
	...
	public function GetFileArrayDirect($file, $arParams=array(), $fieldName='')
	{
		...
		elseif(preg_match('/http(s)?:\/\//', $file))
		{
			...
			if(class_exists('\Bitrix\Main\Web\HttpClient'))
			{
				...
				try{
					...
					$file = $loc = preg_replace_callback('/[^:\/?=&#@%]+/', array('CKDAImportUtils', 'UrlEncodeCallback'), str_replace('\\', '/', $file));
					$arUrl = parse_url($loc);
		...
		if(array_key_exists('type', $arFile))
		{
	...
	public function GetIblockElementValue($arProp, $val, $fsettings, $bAdd = false, $allowNF = false, $allowMultiple = false)
	{
		...
		if(($relField && $arProp['LINK_IBLOCK_ID']) || $relField=='IE_ID')
		{
			...
			$dbRes = \Bitrix\KdaImportexcel\DataManager\IblockElementTable::GetListComp($arFilter, array('ID'), array('ID'=>'ASC'), ($allowMultiple ? false : 1));
	...
	public function GetIblockElementValueEx($arProp, $val, $bAdd = false, $allowNF = false, $allowMultiple = false)
	{
		...
		$arKeys = array_merge(array('ID', 'NAME', 'IBLOCK_SECTION_ID'), array_keys($arElemFields));
	...
	public function ApplyConversions($val, $arConv, $arItem, $field=false, $iblockFields=array())
	{
		...
		if(is_array($arConv))
		{
			$execConv = false;
			$this->currentItemValues = $arItem;
			$prefixPattern = '/(\$\{[\'"])?(#CELL~*\d+#|#CELL\d+[\-\+]\d+#|#CLINK#|#CNOTE#|#HASH#|#FILENAME#|#SHEETNAME#|#IMPORT_PROCESS_ID#|#SEP_SECTION#|#DATETIME#|'.implode('|', $this->rcurrencies).')([\'"]\})?/';
